class Library {
	...
	added(handleList) {
		...
		switch (true) {
			case handleList.Count < 100: {
				let lis = ppt.filterBy ? $.query(handleList, this.filterQuery) : handleList;
				panel.sort(lis);
				...
				if (panel.search.txt) {
					...
					this.validSearch = true;
					try {
						newSearchItems = fb.GetQueryItems(handleList, panel.search.txt);
					} catch (e) {
	...
	rootNames(li, search, treeArtToggle) {
		...
		if (!treeArtToggle || !panel.samePattern) switch (search) {
			case 0:
				if (ppt.libSource) panel.sort(this.list);
		...
		if (!treeArtToggle || !panel.samePattern) {
			switch (tree_type) {
				...
				case 1:
					li.GetLibraryRelativePaths().forEach((v, i) => arr[i] = v.split('\\'));
					break;
			}
	...
	rootNodes(lib_update, process) {
		...
		if (panel.search.txt && (this.upd_search || lib_update === true)) {
			...
			this.none = '';
			try {
				panel.list = fb.GetQueryItems(this.getSearchList(panel.search.txt) || this.list, panel.search.txt);
	...
	updateLibrary(handleList, handleType) {
		...
		switch (handleType) {
			...
			case 1: {
				...
				if (upd_done) break;
				if (ppt.filterBy) { // filter: check if not done
				...
				if (panel.search.txt) { // search: check if not done
					...
					this.validSearch = true;
					try {
						handlesInSearch = fb.GetQueryItems(removeSearchItems, panel.search.txt);
					} catch (e) {
						this.validSearch = false;
					}
