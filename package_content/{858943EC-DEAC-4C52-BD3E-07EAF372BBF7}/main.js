...
function BottomPanel() {
    this.on_paint = function (gr) {
        ...
        var slider_back_color = _RGB(37, 37, 37);
        var slider_bar_color = _RGB(110, 112, 114);
        var slider_bar_hover_color = _RGB(151, 153, 155);
        ...
        // SeekBar
        {
            ...
            if (fb.IsPlaying && fb.PlaybackLength > 0) {
                ...
                gr.FillSolidRect(seek_x, seek_y + p, seekbar_obj.pos(), seek_h - p * 2, _RGBtoRGBA(slider_bar_hover_color, seekbar_obj.hover_alpha));
            }
            ...
            if (!seekbar_obj.drag) {
                ...
                if (g_properties.show_remaining_time && fb.IsPlaying) {
                    ...
                    var track_length = (fb.IsPlaying ? _tfe('[%length%]') : _tf('[%length%]', metadb));
                    seekbar_time_2 = (track_length ? track_length : ' 0:00');
                }
            }

            var slider_text_color = (fb.IsPlaying ? _RGB(130, 132, 134) : _RGB(80, 80, 80));
        ...
        if (g_properties.enable_volume_bar) {
            ...
            gr.FillSolidRect(x, y + p, volume_bar_obj.pos(), h - p * 2, _RGBtoRGBA(slider_bar_hover_color, seekbar_obj.hover_alpha));
        }
    ...
    this.on_size = function (w, h) {
        ...
        if (g_properties.enable_volume_bar) {
            ...
            volume_bar_obj = new _volume(volume_bar_x, volume_bar_y, volume_bar_w, volume_bar_h);
            volume_bar_obj.show_tt = show_tooltips;

            right_pad = this.w - (volume_bar_x - this.x);
        }
        else {
            volume_bar_obj = new _volume(0, 0, 0, 0);
        }

        {
            ...
            seekbar_obj = new _seekbar(seekbar_x, seekbar_y, seekbar_w, seekbar_h);
    ...
    function create_buttons(wx, wy, ww, wh) {
        ...
        buttons.buttons.repeat = new _button(x, y, w, h, repeat_img, repeat_fn, 'Repeat');
        ...
        buttons.buttons.shuffle = new _button(x + (w + p), y, w, h, (plman.PlaybackOrder === g_playback_order.shuffle_tracks) ? button_images.ShuffleTracks : button_images.Shuffle, shuffle_fn, 'Shuffle');

        if (g_properties.enable_volume_bar) {
            var vol_value = _toVolume(fb.Volume);
            var vol_image =
                ((vol_value > 50)
            ...
            buttons.buttons.mute = new _button(ww - 30, y, w, h, vol_image, function () { fb.VolumeMute(); }, vol_value === 0 ? 'Unmute' : 'Mute');
        }
    ...
    function create_button_images() {
        ...
        var default_ico_colors =
            [
                _RGBA(c[0], c[1], c[2], 35),
                _RGBA(c[0], c[1], c[2], 155),
                _RGBA(c[0], c[1], c[2], 105)
            ];

        var accent_ico_colors =
            [
                _RGBA(255, 220, 55, 155),
                _RGBA(255, 220, 55, 225),
                _RGBA(255, 220, 55, 105)
            ];
    ...
    var seekbar_obj = new _seekbar(0, 0, 0, 0);
