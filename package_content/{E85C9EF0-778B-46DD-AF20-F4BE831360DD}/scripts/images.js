class Images {
	constructor() {
		...
		this.style = {
			image: 0,
			rootComposite: ppt.rootNode && ppt.albumArtRootCollage,
	...
	* range(start, end, step) {
	...
	createCacheFolder() {
		...
		$.create(fb.ProfilePath + 'smp-data\\');
		$.create(fb.ProfilePath + 'smp-data\\library-tree-cache\\');
		...
		this.cacheFolder = fb.ProfilePath + 'smp-data\\library-tree-cache\\' + ['front', 'back', 'disc', 'icon', 'artist'][ppt.artId] + (this.saveSize == 250 ? '' : this.saveSize) + '\\';
	...
	metrics() {
		...
		if (stub) this.stub.noImg = this.format(stub, ppt.artId, ['default', 'default', 'circular'][this.style.image], this.im.w, this.im.w, false, 'noImg');
		if (this.root_img) this.stub.root = this.format(this.root_img, ppt.artId, 'default', this.im.w, this.im.w, false, 'root');
		panel.treePaint();
	}
		...
		if (items === 'all') {
			...
			const continue_confirmation = (status, confirmed) => {
				if (confirmed) {
					...
					app.NameSpace(10).MoveHere(fb.ProfilePath + 'smp-data\\library-tree-cache\\'); // remove all saved images & databases if albumArtDiskCache
				}
			}
...
Continue?`, 'Yes', 'No', continue_confirmation);
		...
		const base = fb.ProfilePath + 'smp-data\\library-tree-cache\\' + ['front', 'back', 'disc', 'icon', 'artist'][ppt.artId];
