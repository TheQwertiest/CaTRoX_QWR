class Populate {
	...
	imgView(n) {
		...
		a.forEach(v => b.forEach(w => c.push([v + '^@^' + w])));
		return [...new Set(c)];
	}
	...
	load(list, isArray, add, autoPlay, def_pl, insert) {
		...
		if (plman.IsPlaylistLocked(pln)) return;
	...
	setActions(n, i) {
		switch (n) {
			...
			case 'key':
				...
				break;
			case 'dblClick':
				ppt.dblClickAction = i;
				return;
			case 'send':
				this.autoPlay.send = !this.autoPlay.send;
				ppt.autoPlay = this.autoPlay.send;
				return;
		}
		this.autoPlay = {
	...
	setPlaylistSelection(ix, item) {
		...
		plman.SetPlaylistSelection($.pl_active, this.range(item.item), true);
		this.setFocus = true;
		plman.SetPlaylistFocusItem($.pl_active, this.range(item.item)[0]);
		this.track(false);
		lib.treeState(false, ppt.rememberTree);
	}
