...
function PlaybackPanel() {
    this.on_paint = function (gr) {
        ...
        if (show_volume_bar) {
            ...
            var slider_bar_color = _RGB(190, 192, 194);
            var frame_color = _RGB(200, 200, 200);
    ...
    this.on_size = function (w,h) {
        ...
        if (g_properties.enable_volume_bar) {
            ...
            volume_bar_obj = new _volume(vol_x, vol_y, vol_w, vol_h);
    ...
    function create_buttons(wx, wy, ww, wh) {
        ...
        buttons.buttons.stop = new _button(x, y, w, h, button_images.Stop, function () { fb.Stop(); }, 'Stop');
        x += w + p;

        buttons.buttons.previous = new _button(x, y, w, h, button_images.Previous, function () { fb.Prev(); }, 'Previous');
        x += w + p;

        buttons.buttons.play = new _button(x, y, w, h, !fb.IsPlaying || fb.IsPaused ? button_images.Play : button_images.Pause, function () { fb.PlayOrPause(); }, !fb.IsPlaying || fb.IsPaused ? 'Play' : 'Pause');
        x += w + p;
        ...
        if (g_properties.enable_volume_bar) {
            ...
            buttons.buttons.mute = new _button(x, y + 1, w, h, vol_image, function () { fb.VolumeMute(); }, vol_value === 0 ? 'Unmute' : 'Mute');
            x += w - 5;

            w = button_images.ShowVolume.normal.Width;
            h = button_images.ShowVolume.normal.Height;
            y = wy + Math.floor(wh / 2 - w / 2);
            buttons.buttons.volume = new _button(x, y + 1, button_images.ShowVolume.normal.Width, h, button_images.ShowVolume, function () {
