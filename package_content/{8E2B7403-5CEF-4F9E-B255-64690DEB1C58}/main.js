...
var g_ultra_mini_colors = {
    title:      _RGB(240, 240, 240),
    dummy_text: _RGB(70, 70, 70)
};
...
function UltraMini() {
    ...
    this.on_paint = function (gr) {
        ...
        // Title
        gr.FillGradRect(0, -1, this.w, 40, 270, _RGBA(0, 0, 0, 0), _RGBA(0, 0, 0, 255));
        gr.FillGradRect(0, -1, this.w, 40, 270, _RGBA(0, 0, 0, 0), _RGBA(0, 0, 0, 255));
        ...
        if (panel_alpha !== 0) {
            // Controls
            gr.FillGradRect(0, this.h - playback_h - seekbar_h, this.w, playback_h + seekbar_h + 1, 90, _RGBA(0, 0, 0, 0), _RGBA(0, 0, 0, panel_alpha));
            ...
            var sliderBackColor = _RGBA(37, 37, 37, panel_alpha);
            var sliderBarColor = _RGBA(190, 192, 194, panel_alpha);
            var sliderBarHoverColor = _RGBA(231, 233, 235, Math.min(panel_alpha, seekbar_obj.hover_alpha));
            ...
            if (fb.IsPlaying && fb.PlaybackLength > 0) {
                ...
                gr.FillSolidRect(x + seekbar_obj.pos() - 2, y + p - 1, 4, h - p * 2 + 2, _RGBA(255, 255, 255, panel_alpha));
            }
            ...
            if (show_volume_bar) {
                ...
                var volFrameColor = _RGBA(255, 255, 255, panel_alpha);
    ...
    this.on_size = function () {
        ...
        seekbar_obj = new _seekbar(5, seekbar_y, this.w - 10, seekbar_h);
        seekbar_obj.show_tt = show_tooltips;

        var volume_bar_y = playback_y + Math.floor(playback_h / 2 - volume_bar_h / 2) + 2;
        volume_bar = new _volume(volume_bar_x, volume_bar_y, Math.min(this.w - volume_bar_x - 4, 60), volume_bar_h);
    ...
    function create_buttons(wx, wy, ww, wh) {
        ...
        buttons.buttons.stop = new _button(x, y, w, h, button_images.Stop, function () {
            fb.Stop();
        ...
        x += w + p;
        buttons.buttons.previous = new _button(x, y, w, h, button_images.Previous, function () { fb.Prev(); }, 'Previous');

        x += w + p;
        buttons.buttons.play = new _button(x, y, w, h, (!fb.IsPlaying || fb.IsPaused) ? button_images.Play : button_images.Pause, function () {
        ...
        x += w + p;
        buttons.buttons.next = new _button(x, y, w, h, button_images.Next, function () { fb.Next(); }, 'Next');

        x += w + p;
        volume_bar_x = x + 3;

        var volValue = _toVolume(fb.Volume);
        var volImage = ((volValue > 50) ? button_images.VolLoud : ((volValue > 0) ? button_images.VolQuiet : button_images.VolMute));
        buttons.buttons.mute = new _button(x, y, w, h, volImage, function () { fb.VolumeMute(); }, volValue === 0 ? 'Unmute' : 'Mute');

        x += w - 5;
        buttons.buttons.volume = new _button(x, y + 2, button_images.ShowVolume.normal.Width, h, button_images.ShowVolume, function () {
...
function TitleCycler(on_change_fn_arg) {
    ...
    function cycle_title(skip_callback) {
        ...
        do {
            ...
            var titleText = _tfe(title_query);
            if (titleText.length) {
