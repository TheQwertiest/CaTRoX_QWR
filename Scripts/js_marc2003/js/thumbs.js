			...
			case this.properties.mode.value == 4: // bottom
				...
				this.columns = Math.ceil(this.w / this.properties.px.value);
					...
					this.img = gdi.CreateImage(this.properties.px.value * this.images.length, this.properties.px.value);
                        var temp_gr = this.img.GetGraphics();
                        _.forEach(this.images, _.bind(function (item, i) {
                            _.drawImage(temp_gr, item, i * properties.px.value, 0, this.properties.px.value, this.properties.px.value, image.crop_top);
                        }, this));
                        this.img.ReleaseGraphics(temp_gr);
                        temp_gr = null;
                    }
        ...
        this.rbtn_up = function (x, y) {
			...
			switch (this.properties.source.value) {
                ...
                case 1: // last.fm
                    ...
                    panel.s10.AppendMenuItem(MF_STRING, 4005, "Automatic download");
                    panel.s10.CheckMenuItem(4005, this.properties.auto_download.enabled);

                    _.forEach(this.limits, function (item) {
                        panel.s14.AppendMenuItem(MF_STRING, item + 4010, item);
                    });
                    panel.s14.CheckMenuRadioItem(_.first(this.limits) + 4010, _.last(this.limits) + 4010, this.properties.limit.value + 4010);
